...
```
app/                      the application
  app.py                  Flask: the six-step wizard and a REST API
  capabilities.py          what this machine can measure, and why not what it can't
  docker_api.py            Docker socket client, no dependencies
  rules.py                 membership rules: four criteria, union, exclusions
  generator.py             Jinja2 templates to stack files, plus target_info
  state.py                 load and save config.yaml
  stack/                   templates for the generated stack
    build_overview.py      builds the Grafana dashboard (38 panels)
  templates/               the wizard's HTML
```

The dashboard is generated by a script rather than hand-written JSON — 38 panels of JSON are
unmaintainable otherwise, and adding a panel is three lines.
